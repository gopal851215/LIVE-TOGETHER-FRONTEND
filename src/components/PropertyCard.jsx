      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex flex-wrap gap-2 text-xs mt-2">
          <span className="rounded-full bg-brand-50 px-2 py-1 text-brand-700">
            {property.bhkType || "1BHK"}
          </span>
          <span className="rounded-full bg-slate-100 px-2 py-1 text-slate-600">
            Allowed: {property.allowedMembers}
          </span>
          <span className="rounded-full bg-slate-100 px-2 py-1 text-slate-600">
        ...
        {isCombo && property.occupants?.length > 0 && (
          <div className="mt-3 rounded-xl bg-amber-50 p-3 text-xs text-amber-900 space-y-2">
            <p className="font-semibold">
              Current booking{property.occupants.length > 1 ? "s" : ""} (
              {property.occupants.reduce((sum, occ) => sum + (occ.seatsBooked || 1), 0)} seats):
            </p>
            {property.occupants.slice(0, 2).map((occ, idx) => (
              <div key={idx} className="space-y-0.5 border-b border-amber-100 pb-1 last:border-none">
                ...
                {occ.professionalInfo?.profession && (
                  <p>Profession: {occ.professionalInfo.profession}</p>
                )}
                {occ.professionalInfo?.workplace && (
                  <p>Workplace: {occ.professionalInfo.workplace}</p>
                )}
                {occ.professionalInfo?.age && (
                  <p>Age: {occ.professionalInfo.age}</p>
                )}
                <p className="text-[11px] text-amber-800">
                  Leaving: {new Date(occ.leaveDate).toLocaleDateString()}
                </p>
              </div>
            ))}
            {property.occupants.length > 2 && (
              <p className="mt-1 text-[11px] font-medium text-amber-800">
                +{property.occupants.length - 2} more booking
                {property.occupants.length - 2 > 1 ? "s" : ""}
              </p>
            )}
          </div>
        )}
